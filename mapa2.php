...
// ---------------- MAPA: sempre todas as ativas ----------------
$resultMapa = $conn->query("SELECT * FROM maquinas WHERE status=1 ORDER BY id ASC");
$maquinasMapa = [];
...
// Conta total
if ($filtroId) {
    $stmtCount = $conn->prepare("SELECT COUNT(*) as total FROM maquinas WHERE status=1 AND id = ?");
    $stmtCount->bind_param("i", $filtroId);
} else {
    $stmtCount = $conn->prepare("SELECT COUNT(*) as total FROM maquinas WHERE status=1");
}
$stmtCount->execute();
...
// Busca para tabela
if ($filtroId) {
    $stmt = $conn->prepare("SELECT * FROM maquinas WHERE status=1 AND id = ? LIMIT ?, ?");
    $stmt->bind_param("iii", $filtroId, $offset, $registrosPorPagina);
} else {
    $stmt = $conn->prepare("SELECT * FROM maquinas WHERE status=1 ORDER BY id ASC LIMIT ?, ?");
...
<head>
<meta charset="UTF-8">
<title>Mapa de Máquinas Ativas</title>
...
<body>
<h1 style="text-align:center;">Mapa de Máquinas Ativas</h1>
...
maquinas.forEach(m => {
  let marker = L.marker([m.latitude, m.longitude], {
    icon: L.divIcon({className: 'custom-icon', html: `<div style="color:green;font-size:22px;">⬤</div>`})
  }).bindPopup(`
    <b>ID:</b> ${m.id}<br>
    <b>Nome:</b> ${m.nome}<br>
    <b>Status:</b> <span style="color:green;font-weight:bold;">Ativa</span><br>
...
<h2>Lista de Máquinas Ativas</h2>
